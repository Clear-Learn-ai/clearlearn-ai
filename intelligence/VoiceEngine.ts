...
export class VoiceEngine {
  ...
  private generateSimulationSpeech(content: GeneratedContent, startTime: number): SpeechSegment[] {
    ...
    if (data.parameters && data.parameters.length > 0) {
      const paramText = `You can control ${data.parameters.length} different parameters: ${data.parameters.slice(0, 3).map((p: any) => p.name).join(', ')}.`
      segments.push({
        text: paramText,
  ...
  private generateConceptMapSpeech(content: GeneratedContent, startTime: number): SpeechSegment[] {
    ...
    if (data.nodes && data.nodes.length > 0) {
      const nodeText = `There are ${data.nodes.length} main concepts in this map. Click on any concept to learn more about it.`
      segments.push({
        text: nodeText,
