...
export function ContentDisplay() {
  ...
  const renderContent = () => {
    ...
    switch ((currentContent as any).modality) {
      case 'animation':
        return <AnimationRenderer data={(currentContent as any).data} metadata={(currentContent as any).metadata} />
      
      case 'simulation':
        return <SimulationRenderer data={(currentContent as any).data} metadata={(currentContent as any).metadata} />
      
      case '3d':
        return <Model3DRenderer data={(currentContent as any).data} metadata={(currentContent as any).metadata} />
      
      case 'concept-map':
        return <ConceptMapRenderer data={(currentContent as any).data} metadata={(currentContent as any).metadata} />
          ...
          <div className="p-6">
            <div className="prose max-w-none">
              <div className="text-gray-800 leading-relaxed">
                {(currentContent as any).data?.content || currentContent}
              </div>
            </div>
          </div>
        )
          ...
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                Modality &quot;{(currentContent as any).modality}&quot; is not yet implemented
              </p>
            </div>
          </div>
        )
  ...
  return (
    <div className="learning-card p-6">
        ...
        {currentContent && (
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <span className={cn(
              ...
              {(currentContent as any).modality || 'text'}
            </span>
            <span>Difficulty: {(currentContent as any).metadata?.difficulty || 5}/10</span>
            <span>Duration: {Math.ceil(((currentContent as any).metadata?.estimatedDuration || 300) / 60)}min</span>
          </div>
        )}
      </div>
      ...
      {currentContent && (currentContent as any).metadata?.tags?.length > 0 && (
        <div className="mt-4">
          <div className="text-sm font-medium text-gray-700 mb-2">Related Topics:</div>
          <div className="flex flex-wrap gap-2">
            {(currentContent as any).metadata.tags.map((tag: any, index: any) => (
              <span
                key={index}
