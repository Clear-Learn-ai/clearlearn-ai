...
export function PlumbingFlowDiagram({
  ...
  const initialNodes = useMemo(() => {
    ...
    system.components.forEach((component: PlumbingComponent, index: number) => {
      let nodeType = 'pipe'
      let icon = '🔧'
    ...
    for (let i = 0; i < system.components.length - 1; i++) {
      ...
      edges.push({
        ...
        data: {
          ...
          size: parseFloat(source.size || '1') || 1,
        },
      })
    }
  ...
  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    const component = system.components.find((c: PlumbingComponent) => c.id === node.id)
  ...
  return (
    <div className={className}>
      <ReactFlow
        ...
        nodeTypes={nodeTypes}
        edgeTypes={edgeTypes}
        /* connectionMode intentionally default */
        snapToGrid
        snapGrid={[20, 20]}
