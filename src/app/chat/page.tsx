...
import { useSelectedVideo, useMessages, useIsLoading, useError, useSendMessage, useSetCurrentQuery, useSelectVideo, useSetError, VideoResult, useLearningSession } from '@/lib/store'
...
export default function ChatPage() {
  ...
  const { id: sessionId } = useLearningSession()
  ...
  useEffect(() => {
    // Lazy-load history when session starts
    import('@/lib/store').then(mod => mod.useEducationalTutorStore.getState().loadHistory(sessionId as any)).catch(() => {})
  }, [sessionId])

  return (
    <ProtectedRoute redirectMessage="Sign in to start chatting with TradeAI Tutor and get personalized plumbing guidance">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-xs px-2 py-1 rounded-full border" style={{ borderColor: '#1E0F2E', color: '#1E0F2E' }}>
              Plan: Free
            </div>
          ...
          <div className="max-w-4xl mx-auto p-6">
            {messages.length === 0 ? (
              ...
              <div className="space-y-8">
                <div className="mb-2 text-xs text-center text-yellow-800 bg-yellow-50 border border-yellow-200 rounded-lg p-2">
