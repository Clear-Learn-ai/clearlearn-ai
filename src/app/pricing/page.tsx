              ...
              >
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      if (plan.name === 'Free') {
                        await fetch('/api/plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan: 'free' }) })
                        window.location.href = '/chat'
                        return
                      }
                      const res = await fetch('/api/billing/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan: plan.name === 'Pro' ? 'pro' : 'pro-max' }) })
