...
export class MultiAIOrchestrator {
  ...
  private async createPlan(query: string): Promise<OrchestrationPlan> {
    const desiredMediaTypes = this.determineMediaTypes(query)
    const services: { [key in MediaType]?: AIProvider } = {}
    const availableMediaTypes: MediaType[] = []

    for (const mediaType of desiredMediaTypes) {
      const provider = getServiceForMediaType(mediaType)
      if (this.clients.has(provider)) {
        services[mediaType] = provider
        availableMediaTypes.push(mediaType)
      }
    }
