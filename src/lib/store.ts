...
// Temporary inline types until api.ts moved
export type ChatMessage = { id: string; role: 'user' | 'assistant'; content: string; timestamp: Date; videoResults?: any[] }
type ChatApiRequest = { message: string; sessionId: string; conversationHistory: ChatMessage[] }
...
export const useEducationalTutorStore = create<EducationalTutorStore>()(
  subscribeWithSelector((set, get) => ({
        ...
        try {
          await fetch('/api/history/save', {
            ...
            body: JSON.stringify({
              ...
              videoResults: assistantMessage.videoResults || []
            })
          })
        } catch {}
    ...
    // Load persisted history from Supabase-backed API
    loadHistory: async (sessionId: string) => {
      try {
        const res = await fetch(`/api/history/list?sessionId=${encodeURIComponent(sessionId)}`)
        if (!res.ok) return
